...
export class RegistroMaestrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.activatedRoute.snapshot.params['id'] != undefined){
      ...
      // Ensure materias_json is an array (backend may send a JSON string)
      if (this.maestro) {
        if (!this.maestro.materias_json) {
          ...
          try {
            ...
            this.maestro.materias_json = Array.isArray(parsed) ? parsed : String(this.maestro.materias_json).split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
          } catch (e) {
            this.maestro.materias_json = String(this.maestro.materias_json).split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
          }
        }
      }
    }else{
  ...
  public revisarSeleccion(nombre: string){
    const list = this.maestro && this.maestro.materias_json;
    if (!Array.isArray(list)) return false;
    return list.find((element: any) => element == nombre) !== undefined;
  }
